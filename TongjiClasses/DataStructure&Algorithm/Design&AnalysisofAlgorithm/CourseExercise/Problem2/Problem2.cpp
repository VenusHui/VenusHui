...
template <typename Type> CircularList<Type>::CircularList() {
  listSize = 0;
...
template <typename Type> void CircularList<Type>::remove(int index) {
...
JosephusSolution::JosephusSolution() {
  cout << "现有N个人围成一圈，从第S人开始依次报数，报M的人出局，再由下一个人报"
          "数，如此循环，直到最后剩下K个人为止"
       << endl;
...
void JosephusSolution::input(int &data, const string cueStr, int low,
                             int high) {
  bool loop = true;
  while (loop) {
    cout << "请输入" << cueStr << "：";
