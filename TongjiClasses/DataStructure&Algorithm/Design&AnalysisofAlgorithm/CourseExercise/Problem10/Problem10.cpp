...
SortFunctions::SortFunctions() {
  ...
  cout << "**            9 --- 退出程序           **" << endl;
  cout << "=========================================" << endl;
  sortNames = new string[FunctionNum + 1]{
...
void SortFunctions::MergeSort(const int low, const int high) {
  function<void(const int, const int, const int)> Merge = [&](const int left,
                                                              const int mid,
                                                              const int right) {
...
void SortFunctions::PrintDetail() {
  ...
  cout << sortNames[oper - '0'] << "排序所用时间" << setiosflags(ios::fixed)
       << setprecision(6)
       << (static_cast<double>(finish) - static_cast<double>(start)) /
              CLOCKS_PER_SEC
       << " s" << endl;
  cout << sortNames[oper - '0'] << "排序交换次数" << compCount << endl << endl;
}
