...
Maze::Maze() {
  ...
  cout << "if your want to use the fixed maze map, please INPUT 1, or INPPUT "
          "ANYTHING ELSE to input the map by yourself!"
       << endl;
  cin >> oper;
  row = col = 6;
  startX = 5;
  ...
  if (oper != 1) {
    cout << "Please input the Maze ,'#' refers to unavilible node while '0' "
            "refers to avilible node:"
         << endl;
    for (int i = 0; i < row; i++) {
      char ch = getchar();
      if (ch != '\n') {
    ...
    if (MazeMap[startX][startY] != '0' ||
        MazeMap[endX][endY] != '0') // 选择的起点、终点为不可走的
    {
      cout << "the start node or the end node is not avilible, please check "
              "the input"
           << endl;
    }
  }
...
bool Maze::dfs(int start_x, int start_y) {
  // 触碰边界
  if (start_x < 0 || start_y < 0 || endX < 0 || endY < 0 || start_x >= row ||
      start_y >= col || endX >= row || endY >= col) {
    return false;
  }
  // 不可访问或已访问过
