...
template <typename Type> Stack<Type>::Stack() { topPtr = nullptr; }
...
template <typename Type> void Stack<Type>::push(Type elem) {
...
void ExpConvert::createTree() {
  try {
    ...
    dataStack.pop();
    BiTreeNode<string> *curOper =
        new BiTreeNode<string>(operatorStack.top(), right, left);
    operatorStack.pop();
