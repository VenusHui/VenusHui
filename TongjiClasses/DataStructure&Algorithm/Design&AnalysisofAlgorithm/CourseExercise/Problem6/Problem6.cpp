...
struct TreeNode // 树结点，使用孩子兄弟表示法
{
  ...
  TreeNode(Type x, TreeNode *firstChild, TreeNode *nextSibling)
      : val(x), firstChild(firstChild), nextSibling(nextSibling) {}
};
...
template <typename Type> bool Tree<Type>::remove(TreeNode<Type> *pNode) {
  if (pNode == nullptr) {
    return false;
  }
  if (pNode->firstChild != nullptr) {
    function<void(TreeNode<Type> *)> RemoveSubTree =
        [&](TreeNode<Type> *rNode) {
          if (rNode == nullptr) {
            return;
          } else {
...
void FamilyTree::ChangeName() {
  ...
  if (curNode != nullptr) {
    ...
    if (find(root, newName) != nullptr) {
      cout << "已经存在叫" << newName << "的家庭成员，更名会导致家谱混乱"
           << endl;
      return;
    }
    curNode->val = newName;
