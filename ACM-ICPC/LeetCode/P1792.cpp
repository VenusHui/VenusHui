...
private:
  ...
  inline void down(int x) {
    int t = x;
    while (x * 2 < size && heap[x * 2].second > heap[t].second)
      t = x * 2;
    while (x * 2 + 1 < size && heap[x * 2 + 1].second > heap[t].second)
      t = x * 2 + 1;
    if (t != x)
...
public:
  double maxAverageRatio(vector<vector<int>> &classes, int extraStudents) {
    ...
    while (extraStudents--) {
      ...
      cnt.second = 1.0 * (cnt.first[0] + 1) / (cnt.first[1] + 1) -
                   1.0 * cnt.first[0] / cnt.first[1];
      insert(cnt);
    }
    double ans = 0;
