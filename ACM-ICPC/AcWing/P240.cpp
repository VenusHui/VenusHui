...
int main() {
  ...
  cin >> n >> k;
  for (int i = 0; i <= n; i++)
    disjoint_set.push_back(make_pair(i, 0));
  int ans = 0;
  while (k--) {
    int d, x, y;
    cin >> d >> x >> y;
    ...
    if (d == 1) {
      if (px.first == py.first && (px.second - py.second) % 3)
        ans++;
      else if (px.first != py.first) {
        disjoint_set[px.first].second =
            disjoint_set[y].second - disjoint_set[x].second;
        disjoint_set[px.first].first = py.first;
      }
    } else if (d == 2) {
      if (px.first == py.first && (px.second - py.second - 1) % 3)
        ans++;
      else if (px.first != py.first) {
        disjoint_set[px.first].second =
            disjoint_set[y].second - disjoint_set[x].second + 1;
        disjoint_set[px.first].first = py.first;
      }
    }
  }
