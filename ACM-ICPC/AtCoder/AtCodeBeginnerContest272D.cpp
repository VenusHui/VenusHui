...
int main() {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        for (int i = 0; i < size; i++) {
            pair<int, int> nxt = make_pair(cnt.first + res[i].first, cnt.second + res[i].second);
            if (nxt.first >= 0 && nxt.first < n && nxt.second >= 0 && nxt.second < n && ans[nxt.first][nxt.second] == -1) {
